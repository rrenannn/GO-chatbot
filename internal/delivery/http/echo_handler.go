...
func (h *HttpHandler) RegisterRoutes(e *echo.Echo) {
	...
	// Middleware aplicado por rota (em vez de sub-grupos com prefixo vazio,
	// que no Echo podem vazar o middleware para rotas que não deveriam tê-lo).
	api.GET("/whatsapp/qr", h.StreamQRCode, h.authMiddleware)
	api.GET("/whatsapp/status", h.WhatsAppStatus, h.authMiddleware)
	api.POST("/trigger-post-sale", h.TriggerPostSale, h.authMiddleware, h.requireAdmin)
	api.POST("/broadcast", h.Broadcast, h.authMiddleware, h.requireAdmin)
	api.POST("/impersonate", h.Impersonate, h.authMiddleware, h.requireAdmin)
}
